...
if('splitPlot' %in% names(args) & 'splitPpm' %in% names(args)){
    splitPlot = ifelse(args[['splitPlot']]=='Y', T, F)
    if (splitPlot) splitPpm = as.numeric(args[['splitPpm']])
    else splitPpm = 0
...
make.MDoutput = function(plts){
  ...
  intro = ''
  prePlt1 = ''
  plts = sapply(plts, function(x)  sprintf('![](%s)\n', x))
  
  output = c(header, intro, prePlt1, plts)
  output
}
...
if (splitPlot){
    starts = seq(toplt[1], toplt[2], by=-splitPpm)
    ends = starts - splitPpm
    starts = starts + 0.05
    ends = ends - 0.05
    
    # make sure starts and ends do not run over
    if (starts[1] > toplt[1]) starts[1] = toplt[1]
    if (starts[length(starts)] > toplt[2]) {
        starts = starts[1:(length(starts)-1)]
        ends = ends[1:(length(ends)-1)]
        }
    if (ends[length(ends)] < toplt[2]) ends[length(ends)] = toplt[2]
    
    plt = rep('', length(starts))
    
    for (i in 1:length(plt)){
        
        pltTmp = paste0(outdir,'/NMRplot', i,'.png')
        plt[i] = paste0('NMRplot', i,'.png')
        png(pltTmp, width=15, height=9, units= 'in', res=300)
        plotNMRspectra(data_, scale, toPlot=c(starts[i], ends[i]), 
                       spread = spreadPar,
